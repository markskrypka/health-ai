...
for i in $(seq 1 200); do
  ...
  busy=$(.venv/bin/python scripts/scored.py status | grep -c "active run: True" || true)
  if [ "$busy" = "0" ] && { [ "$active" = "0" ] || [ "$active" = "down" ]; }; then break; fi
  echo "waiting for the call in flight to finish…"; sleep 5
done
rm -f logs/.run-in-flight
[ "${active:-down}" = "down" ] && bash scripts/restart.sh && touch logs/.hold
pkill -x ngrok || true
...
.venv/bin/python scripts/practice.py endpoint
rm -f logs/.hold
caffeinate -dims .venv/bin/python -u scripts/scored.py loop 2>&1 | tee -a logs/scored-loop.log
