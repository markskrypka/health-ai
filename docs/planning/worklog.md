...
## 2026-09-20 · fix · after a registration the agent says what is true — it no longer invents why it cannot book
By: Mark Skrypka
Why: Mark's first spoken calls from the web page (04:32, 12 minutes; 04:44; 04:49). He registered, was told "You are now registered. How can I help you today?", asked to book — and the agent searched with a patient id it made up (code refused), looked him up (nobody: the clinic's directory never changes, a registration is only a record sent at hang-up, and on the web line not even that), and then explained it with reasons it invented: "your registration was not completed", "I need a valid Spanish phone number". It registered him a second time, told him "clinic rules require registration first before booking. Goodbye." and once tried to discard the registration (code refused). Not booking for a new patient is the organizers' rule ("register: nothing is booked"; a BOOK beside a REGISTER fails the case) — the scored callers hang up after registering, so what comes after had never run. The jury will do what Mark did, and marks an invented rule down hard.
How: no prompt change — the tools say what is true at each of those moments (`tools._AFTER_REGISTERING`): `register_patient`'s answer (done, nothing missing; the first appointment is booked on the next call; do not look up, search or register again; never say it failed; the one reason to give if asked), `find_patient` finding nobody after a registration on this call ("that is how it should be"), `find_slots` with an id that does not exist ("there is no patient_id for someone registered on this call"), and a refusal recorded after a registration ("the registration stands"). Calls without a registration are told what they always were. Verified: 5 new tests (135 pass); a scripted web call that registers and then asks to book, twice: "You are all registered… call us back to book your first appointment", then "your registration is complete, but your first appointment can only be booked on your next call" — one REGISTER recorded, no second registration, 130 s. Runs on the web line (7861) now; the phone line gets it only with a restart, which is Mark's decision. Also seen in those calls and not changed: Spanish answers in the middle of an English call were answered in English (no language switch logged) — a prompt matter for the voice agent.
Ref: 494e790
